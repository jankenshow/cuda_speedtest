...
__global__ void mandelbrot_kernel(float* out, float* r_vals, float* i_vals,
        int size_r, int size_i, int max_iter, float upper_bound) {
...
int main() {
    ...
    float range_step_r = (abs(range_max) + abs(range_min)) / (float)(size_r - 1);
    float range_step_i = (abs(range_max) + abs(range_min)) / (float)(size_i - 1);

    printf("サイズ = %d x %d, 最大イテレーション  = %d, 上限 = %f\n", size_r, size_i, max_iter, upper_bound);
    ...
    cudaMemcpy(d_r_vals, r_vals, sizeof(float) * size_r, cudaMemcpyHostToDevice);
    cudaMemcpy(d_i_vals, i_vals, sizeof(float) * size_i, cudaMemcpyHostToDevice);
    cudaMemcpy(d_out, out, sizeof(float) * size_r * size_i, cudaMemcpyHostToDevice);

    cudaEventRecord(memh2d_stop);
    cudaEventSynchronize(memh2d_stop);

    dim3 grid(std::ceil(size_r / thread_size), std::ceil(size_i / thread_size), 1);
    dim3 threads(thread_size, thread_size, 1);

    mandelbrot_kernel<<<grid, threads>>>(d_out, d_r_vals, d_i_vals, size_r, size_i, max_iter, upper_bound);
    // cudaDeviceSynchronize();

    cudaEventRecord(kernel_stop);
    cudaEventSynchronize(kernel_stop);

    cudaMemcpy(out, d_out, sizeof(float) * size_r * size_i, cudaMemcpyDeviceToHost);
